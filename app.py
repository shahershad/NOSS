# app.py — Streamlit UI (repeated counting; verbs required ONLY for WORK ACTIVITIES)
import re
import html
...
from core.scoring import (
    build_regex_and_maps,            # keyword regex + maps (merges custom keywords)
    default_weights, default_thresholds,
    find_and_count_variants,         # used for aggregates
    compute_field_scores_dynamic,    # supports verbs_required_fields
)
...
# NEW: thresholds for deciding pass at CU/document level
if "cu_pass_threshold" not in st.session_state:
    st.session_state.cu_pass_threshold = 50  # %
if "doc_share_threshold" not in st.session_state:
    st.session_state.doc_share_threshold = 50  # %
...
VERBS_REGEX = build_verb_regex()  # compiled verb regex (used only for WA)
...
def highlight_text_web(text: str, *, use_verbs: bool):
    ...
    # collapse hard newlines so pasted content stays a paragraph
    text = re.sub(r'[\r\n]+', ' ', text)
...
with tab1:
    ...
    if uploaded is not None:
        ...
        CU_T  = st.session_state.cu_pass_threshold
        DOC_T = st.session_state.doc_share_threshold
        ...
        pct_gt = (gt_geT/total_cus*100.0) if total_cus else 0.0
        pct_ir = (ir_geT/total_cus*100.0) if total_cus else 0.0
        if pct_gt >= DOC_T and pct_ir >= DOC_T:
            ...
            final_category = "MATCHED LESS {0}%".format(DOC_T) if (gt_geT or ir_geT) else "UNMATCHED"

        st.markdown(f"### Document-level Share of CUs ≥ {CU_T}%")
        st.table({
            "Metric":   [f"IR ≥ {CU_T}% CU", f"GT ≥ {CU_T}% CU"],
            "Value":    [ir_geT, gt_geT],
            "Total CU": [total_cus, total_cus],
            "Percent":  [f"{pct_ir:.0f}%", f"{pct_gt:.0f}%"],
        })
        st.markdown(f"**Category (new method, doc threshold {DOC_T}%):** {final_category}")
        ...
        pdf_bytes = build_pdf_for_single(
            ...
            verbs_regex=VERBS_REGEX,
            verbs_required_fields={"WORK ACTIVITY"},     # ONLY WA needs verbs
            cu_pass_threshold=CU_T,
            doc_share_threshold=DOC_T
        )
        ...
        st.caption("Yellow = Green Tech (GT), Green = Industrial Revolution (IR). WA blue sentence = verb before keyword; verb is red.")
        ...
        for idx, (cu, gt_scores, ir_scores, total_gt, total_ir) in enumerate(per_cu_cache, 1):
            ...
            cu_title_html = highlight_text_web(cu.get("CU TITLE",""), use_verbs=False)
            cu_desc_html  = highlight_text_web(cu.get("CU DESCRIPTOR",""), use_verbs=False)

            # Split WA/PC into items
            ...
            # WA: verb-aware | PC: keywords-only
            wa_items_html = [highlight_text_web(x, use_verbs=True)  for x in wa_raw]
            pc_items_html = [highlight_text_web(x, use_verbs=False) for x in pc_raw]
...
with tab2:
    ...
    with colB:
        st.markdown("**Thresholds (full mark when reached for each field)**")
    ...
    cu_thr  = st.slider("Per-CU pass threshold (%)", min_value=0, max_value=100, value=st.session_state.cu_pass_threshold, step=1, help="A CU counts as '≥ threshold' when its GT/IR total match meets or exceeds this percentage.")
    doc_thr = st.slider("Document share threshold (%)", min_value=0, max_value=100, value=st.session_state.doc_share_threshold, step=1, help="Share of CUs that must meet the CU threshold for GT/IR to classify the whole document.")

    st.caption("Counting mode is fixed to **Repeated (total occurrences)**. Verb-before-keyword applies **only** to WORK ACTIVITIES.")
    ...
    if st.button("Apply settings"):
        ...
        st.session_state.cu_pass_threshold  = int(cu_thr)
        st.session_state.doc_share_threshold = int(doc_thr)
    ...
    if st.button("📄 Generate PDF & Report", key="manual_generate"):
        ...
        for cu in cu_blocks:
            gt_scores, ir_scores = compute_field_scores_dynamic(
                ...
                GREEN_BASE_SET_S, IR_BASE_SET_S,
                verbs_regex=VERBS_REGEX,
                verbs_required_fields={"WORK ACTIVITY"}
            )
            if sum(gt_scores.values()) >= CU_T: gt_geT += 1
            if sum(ir_scores.values()) >= CU_T: ir_geT += 1
