# core/keywords.py — canonical keyword bases + ONLY explicit variants (no auto expansions)

import re
from collections import Counter

# === Canonical base lists (unchanged) ===
GREEN_BASE = [
    ...
    "environmental, social and governance","predictive maintenance","reuse","sustainable development","electric vehicle car"
]
...
# ---- Variant seeds (as provided; no automatic expansions added) ----
VARIANTS_GREEN_SEED = {
    ...
    "electric vehicle car": ["ev", "evc", "electric vehicle"],
}
...
# ============================================================
# 2b) BAHASA MELAYU VARIANTS (map to the same English bases)
# ============================================================
...
BM_GREEN_VARIANTS = {
    ...
    "electric vehicle car": ["kereta elektrik"]
}
...
# Merge BM variants into seeds (kept as-is)
...
# -------- No-automation variant builders ----------

def expand_phrase_variants(phrase: str):
    """
    NO automatic expansion (no plural/tense/hyphen/space variants).
    Return exactly the phrase (lowercased).
    """
    return { (phrase or "").strip().lower() }

def build_variant_map(base_list, seed_dict):
    """
    Build the variant_to_base map using ONLY:
      - the base itself
      - explicit variants provided in seed_dict (already includes BM variants)
    """
    ...
    for base in base_set:
        ...
        if base in seed_dict:
            seeds.update(v.strip().lower() for v in seed_dict[base] if v.strip())
        for v in seeds:
