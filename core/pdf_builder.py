...
def _split_sentences_for_pdf(text):
    ...
    if not text:
        return []
    # NEW: collapse newlines from pasted content so it stays a paragraph
    text = re.sub(r'[\r\n]+', ' ', text)
    parts = re.split(r'(?<=[\.\!\?\:;])\s+|\n+', text)
...
def _hilite_pdf_with_verbs(text, variants_regex, variant_to_base, verbs_regex,
    ...
    # NEW: collapse newlines so we treat it as flowing paragraph
    text = re.sub(r'[\r\n]+', ' ', text)
...
def build_pdf_for_single(profile_data, cu_blocks, *,
    ...
    for i, cu in enumerate(cu_blocks, 1):
        gt_scores, ir_scores = compute_field_scores_dynamic(
            cu, weights, thresholds, count_mode,
            variants_regex, variant_to_base,
            green_bases, ir_bases,
    ...
    flowables.append(Paragraph("<b>Document-level Share of CUs ≥ 50%</b>", styleH))
    flowables.append(Spacer(1, 0.2 * cm))
    total_cus = len(per_cu_scores)
    gt_ge50   = sum(1 for (_, _, tg, ti) in per_cu_scores if tg >= 50)
    ir_ge50   = sum(1 for (_, _, tg, ti) in per_cu_scores if ti >= 50)
    doc_rows = [
        ["Metric", "Value", "Total CU", "Percent"],
        ["IR ≥ 50% CU", str(ir_ge50), str(total_cus), f"{(ir_ge50/total_cus*100.0 if total_cus else 0):.0f}%"],
        ["GT ≥ 50% CU", str(gt_ge50), str(total_cus), f"{(gt_ge50/total_cus*100.0 if total_cus else 0):.0f}%"],
    ]
    doc_table = fullwidth_centered_table(doc_rows, [0.4, 0.2, 0.2, 0.2])
    flowables.append(doc_table)
    flowables.append(Spacer(1, 0.2 * cm))
    flowables.append(Paragraph(f"<b>Category (new method):</b> {final_category}", styleN))
    flowables.append(Spacer(1, 0.4 * cm))
    ...
    for cu in cu_blocks:
        for field in ["CU TITLE", "CU DESCRIPTOR", "WORK ACTIVITY", "PERFORMANCE CRITERIA"]:
            ...
            if use_verbs:
                ...
                # examples only from qualifying sentences
                for sent in _split_sentences_for_pdf(text):
                    slow = sent.lower()
    ...
    for i, (cu, sc) in enumerate(zip(cu_blocks, per_cu_scores), 1):
        ...
        if use_verbs and verbs_regex:
            cu_title = _hilite_pdf_with_verbs(
                cu.get("CU TITLE", ""), variants_regex, variant_to_base, verbs_regex, green_bases, ir_bases
            )
            cu_desc = _hilite_pdf_with_verbs(
                cu.get("CU DESCRIPTOR", ""), variants_regex, variant_to_base, verbs_regex, green_bases, ir_bases
            )
        else:
